...
def get_bunker_items(keys, num_items):
    ...
    items_lst = [[items for items in input().split(' - ')]
                 for _ in range(num_items)]

    for i in range(len(items_lst)):
        for j in range(len(items_lst[0])):
            if j == 0:
                items_category_dict[items_lst[i][j]].append(
                    items_lst[i][j + 1])

    quantity_quality_lst = [row[indx]
                            for row in items_lst for indx in range(len(row)) if indx == 2]
